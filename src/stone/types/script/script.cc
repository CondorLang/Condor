...
namespace internal{
	...
	void Script::RunInternalScript(Isolate* isolate, std::string hex, std::string _name, std::string sub, bool isHex){
		...
		Stone::Isolate* iso = CAST(Stone::Isolate*, isolate);
		int len = hex.length();
		...
		Stone::String* str = Stone::String::New(iso, newString.c_str());
		String* iStr = CAST(String*, str);
		iStr->name = _name;
		iStr->SetInternal();

		Stone::Context* c = CAST(Stone::Context*, isolate->GetContext());
		Stone::Script* script = Stone::Script::Compile(c, str);
		if (!script->HasError()){
	...
	void Script::LoadIncludes(){
		...
		for (int i = 0; i < parser->includes.size(); i++){
			...
			Stone::Isolate* iso = CAST(Stone::Isolate*, isolate);
			Stone::Context* ctxt = CAST(Stone::Context*, context);
			Stone::String* str = Stone::String::NewFromFile(iso, c.c_str());
			Stone::Script* script = Stone::Script::Compile(ctxt, str);
			script->Run();
		}
	}
