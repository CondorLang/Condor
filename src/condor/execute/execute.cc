	...
	namespace internal{
		...
		ASTLiteral* Execute::EvaluateFuncCall(ASTFuncCall* call){
			...
			if (returns.size() > 0) {
				...
				ASTLiteral* lit = returns[0]->GetLocal(false);
				CHECK(lit != NULL);
		...
		ASTLiteral* Execute::EvaluateBinary(ASTBinaryExpr* binary){
			...
			if (binary->op == PERIOD && binary->right->type == FUNC_CALL && ((ASTFuncCall*) binary->right)->func->HasVisibility(STATIC)){
				...
				ASTLiteral* lit = EvaluateFuncCall((ASTFuncCall*) binary->right);
				FormatLit(lit);
				return lit;
			}
			else{
		...
		void Execute::Assign(ASTBinaryExpr* binary){
			...
			switch (type){
				...
				case DIV_ASSIGN: case MUL_ASSIGN: {
					...
					ASTLiteral* local = var->GetLocal(false);
					CHECK(local != NULL);
				...
				case LITERAL: {
					...
					if (lit->member != NULL) {
						...
						if (value != NULL && value->litType == STRING) { // string array
							...
							ASTLiteral* result = ASTLiteral::New(isolate);
							result->litType = CHAR;
							result->value = value->value.at((unsigned long long int) member->calc);
							return result;
						}
						else if (value != NULL){
							ASTLiteral* result = ASTLiteral::New(isolate);
							ASTLiteral* val = EvaluateValue(lit->var); // reset value
							result->litType = val->litType;
							ASTArray* ary = (ASTArray*) val;
							int spot = (int) member->calc;
							ASTNode* n = ary->members.at((unsigned long long int) spot);
